    ...
    try {
        ...
        if (command == '+owner') { // add owner number
            ...
            if (data.premium) {
                ...
                data.limitGame = 500
                data.premium = true
                data.expired = (new Date() * 1) + (86400000 * 30 * 2)
        ...
        let is_owner = [client.decodeJid(client.user.id)
                .split`@` [0], global.owner, ...global.db.setting.owners].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net')
            .includes(jid)
        ...
        if (is_user.find(v => v.jid == jid)
            .banned) return client.reply(m.chat, Func.texted('bold', `🚩 Target already banned.`), m)
        is_user.find(v => v.jid == jid)
            .banned = true
        let banned = is_user.filter(v => v.banned)
            .length
        ...
        if (!is_user.some(v => v.jid == jid)) return client.reply(m.chat, Func.texted('bold', `🚩 User data not found.`), m)
        if (!is_user.find(v => v.jid == jid)
            .banned) return client.reply(m.chat, Func.texted('bold', `🚩 Target not banned.`), m)
        is_user.find(v => v.jid == jid)
            .banned = false
        let banned = is_user.filter(v => v.banned)
            .length
