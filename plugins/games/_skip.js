neoxr.create(async (m, {
  ...
  try {
     ...
     if (command == 'fiboskip') {
        ...
        return client.sendButtonText(m.chat, teks, `${global.botname}`, playQuiz).then(() => {clearTimeout(client.quiz[id][2])
          delete client.quiz[id]}

        //! SEND MESSAGE WITH TEKS 
        // return client.reply(m.chat, Func.texted('bold', teks), m).then(() => {clearTimeout(client.quiz[id][2])
        //    delete client.quiz[id]}
        )
