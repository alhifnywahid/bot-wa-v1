neoxr.create(async (m, {
   ...
   try {
      ...
      teks += `Lengkapi urutan nomor di bawah ini :\n`
      teks += `➠ ${isD.map(v => v).join(' ').replace(RegExp(isS, 'i'), '_')}\n\n`
      teks += `Timeout : [ *${((timeout / 1000) / 60)} menit* ]\n`
      teks += `Balas pesan ini untuk menjawab, kirim *${prefix}fiboskip* untuk menghapus sesi.`
      client.deret[id] = [
         await client.reply(m.chat, teks, m),
         isS, 3,
         setTimeout(() => {
            if (client.deret[id]) client.reply(m.chat, `*Waktu habis!*, jawabannya adalah : *${client.deret[id][1]}*`, client.deret[id][0])
