neoxr.create(async (m, {
  ...
  try {
     ...
     if (m.isGroup) {
        if (body && hint.includes(body.toLowerCase())) return m.reply('Hai, saya Megachan Bot\Apakah Anda ingin mengobrol dengan saya? kirim *Megachan* untuk membuat sesi obrolan.')
        for (let jid of [...new Set([...(m.mentionedJid || [])])]) {
           if (jid != client.decodeJid(client.user.id)) continue
           if (!m.fromMe) return m.reply('halo kak mau curhat ama simi ketik *Megachan* untuk memulai sesi.')
        }
        if (body && body.toLowerCase() == 'Megachan' && !room) {
        ...
        } else if (body && body.toLowerCase() == 'Megachan' && room) return m.reply('Anda telah berada dalam sesi obrolan.')
        ...
        if (body && hint.includes(body.toLowerCase())) return m.reply('Hi, i\'am Megachan Bot')
        var json = await Scraper.simsimiV2(body)
        if (!json.status) {
           var json = await Scraper.simsimiV2(body)
           if (!json.status) {
