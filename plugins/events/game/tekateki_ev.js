neoxr.create(async (m, {
   ...
   try {
         ...
         if (m.quoted && /tebakskip/i.test(m.quoted.text)) {
            if (!(id in client.tebaktebakan) && /tebakskip/i.test(m.quoted.text)) return client.reply(m.chat, Func.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}tebakskip_ untuk mendapatkan soal baru.`), m)
            if (m.quoted.id == client.tebaktebakan[id][0].key.id) {
